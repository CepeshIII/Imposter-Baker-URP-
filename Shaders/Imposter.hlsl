...
void ImposterVertex(inout ImposterData imp)
    ...
    void ImposterSample(ImposterData imp, SamplerState ss, out float4 outAlbedo, out float4 outNormal)//, out float depth )
    {
    ...
    void Vertex_half(half3 inPositionOS, half2 inUV, out half3 outPositionOS, out half4 outUVGrid, out half4 outPlane0, out half4 outPlane1, out half4 outPlane2)
    {
        ImposterData imp = (ImposterData)0;
...
SAMPLER(SamplerState_Linear_Repeat);
    ...
    void Fragment_half(float3 inNormalWS, float3 inTangentWorld, float3 inBiTangentWS, float4 inUVGrid, float4 inPlane0, float4 inPlane1, float4 inPlane2,  out float3 outColor, out float3 outNormal, out float outAlpha)
    {
        SamplerState ss = SamplerState_Linear_Repeat;
